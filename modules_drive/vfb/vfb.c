...
{
	if(remap_pfn_range(vma,vma->vm_start,virt_to_phys(vfbdev.buf) >> PAGE_SHIFT,vma->vm_end - vma->vm_start,vma->vm_page_prot))
	{
		return -EAGAIN;
	}
	return 0;
...
ssize_t vfb_read(struct file *filp,char __user *buf,size_t count,loff_t *pos)
{
	...
	int ret = copy_to_user(buf,vfbdev.buf,len);
	return len - ret;
}
